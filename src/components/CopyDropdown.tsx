...
export default function CopyDropdown({ symbol, className }: CopyDropdownProps) {
  ...
  // 生成各种格式的复制内容
  const getFormats = (sym: Symbol): CopyFormat[] => [
    {
      label: t('copy.symbol'),
      value: sym.symbol,
      description: '直接复制符号'
  ...
  const formats = getFormats(symbol);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          size="sm"
          className={cn(
            "w-full transition-all duration-200 cursor-pointer text-xs h-7 border-2",
            copied 
